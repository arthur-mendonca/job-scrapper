...
import { CollectionRunRepository, type CollectionRunError } from '../persistence/collection-run.repository.js';
...
import type { CollectionCycleSummary, ProcessRawItemsResult, SourceRunMetrics } from './pipeline.types.js';
...
export class CollectionCycle {
  constructor(
    ...
    private readonly collectionRunRepository = new CollectionRunRepository(),
    private readonly notificationService = new NotificationService()
  ) {}

  async run(): Promise<CollectionCycleSummary> {
    const startedAt = new Date();
    const run = await this.collectionRunRepository.start(startedAt);
    const errors: CollectionRunError[] = [];
    ...
    try {
      ...
      for (const collector of this.collectors) {
        ...
        try {
          ...
          collectorFailures += 1;
          errors.push({ source: collector.name, message: errorMessage(error) });
          sourceMetrics.set(collector.name, {
            source: collector.name,
            sourceTrustScore: 0,
      ...
      const result = await this.processRawItems(rawItems);

      // Merge source metrics from processRawItems
      for (const [key, value] of result.sourceMetrics) {
        const existing = sourceMetrics.get(key);
        if (existing) {
          existing.normalizedItems += value.normalizedItems;
          existing.acceptedItems += value.acceptedItems;
          existing.rejectedItems += value.rejectedItems;
          existing.sourceTrustScore = Math.max(existing.sourceTrustScore, value.sourceTrustScore);
          existing.trustBucket = trustBucket(existing.sourceTrustScore);
        } else {
          sourceMetrics.set(key, value);
        }
      }

      await this.notificationService.sendEmailDigest(result.highScoringPersistedJobs);
      ...
      const summary: CollectionCycleSummary = {
        ...
        rediscoveredJobs: result.rediscoveredJobs,
        highScoringJobs: result.highScoringJobs,
        notificationsSent: result.notificationsSent,
        collectorFailures,
        sourceMetrics: [...sourceMetrics.values()]
      };

      await this.collectionRunRepository.finish({
        id: run.id,
        status: collectorFailures > 0 ? 'partial_failure' : 'success',
        summary,
        errors
      });
      ...
      await this.collectionRunRepository.fail({
        id: run.id,
        error,
        errors
      });
  ...
  /**
   * Process raw job items through the normalize → filter → dedup → persist → notify pipeline.
   * Reusable by both the main collection cycle and dynamic search scheduler.
   */
  async processRawItems(rawItems: RawJobItem[]): Promise<ProcessRawItemsResult> {
    const sourceMetrics = new Map<string, SourceRunMetrics>();
    ...
    for (const job of normalizedJobs) {
      ...
      sourceMetrics.set(job.source, metric);
      ...
      if (filter.accepted) {
        ...
        metric.acceptedItems += 1;
      } else {
        rejectedJobs += 1;
        metric.rejectedItems += 1;
